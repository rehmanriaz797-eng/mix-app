...
const MarketplaceItemPage = () => {
    ...
    const handleMessageSeller = async () => {
        if (!user) return;
        try {
            const chatId = await marketplaceService.contactSeller(user.id, item.user_id, item.id);
            // Navigate to Connect app with this chat active? 
            // Currently our messaging app is on /connect. 
            // We'd need to pass state or param to open specific chat.
            // For now, redirect to connect and user can find it in top list or we implement a route param in Messaging.tsx
            navigate('/connect'); 
        } catch (e) {
