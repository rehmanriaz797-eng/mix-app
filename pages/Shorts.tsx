            ...
            <div className="flex-1 w-full snap-y snap-mandatory overflow-y-scroll scrollbar-hide bg-black overscroll-none scroll-smooth touch-pan-y">
                {shorts.map((short, idx) => (
                    <ShortItem 
                        ...
                        onDescriptionClick={() => setDescriptionShortId(short.id)}
                        onVisible={() => {
                            setCurrentIndex(idx);
...
const DescriptionDrawer = ({ short, onClose }: { short: SortShort, onClose: () => void }) => {
    // Add missing useNavigate hook to handle channel navigation from description
    const navigate = useNavigate();
        ...
        <div className="fixed inset-0 z-[210] flex items-end justify-center animate-in fade-in duration-300">
            ...
            <div className="relative w-full max-w-lg bg-[#141414] rounded-t-[2.5rem] flex flex-col h-[70vh] shadow-[0_-30px_100px_rgba(0,0,0,1)] border-t border-white/10 overflow-hidden animate-in slide-in-from-bottom duration-500">
                ...
                <div className="flex-1 overflow-y-auto px-8 py-10 space-y-10 scrollbar-hide pb-32">
                    ...
                    <div 
                        onClick={(e) => { e.stopPropagation(); navigate(`/channel/@${short.profiles?.username?.toLowerCase()}`); }}
                        className="flex items-center justify-between p-6 bg-white/[0.02] border border-white/5 rounded-[2rem] hover:bg-white/5 transition-all cursor-pointer"
                    >
...
const ShortItem: React.FC<{ 
...
}> = ({ short, isActive, isMuted, playlistsVersion, onVisible, onCommentClick, onSaveClick, onShareClick, onToggleMute, onDescriptionClick }) => {
    const navigate = useNavigate();
    ...
    const triggerSeekFeedback = (dir: 'forward' | 'backward') => {
        ...
        if (skipResetTimerRef.current) window.clearTimeout(skipResetTimerRef.current);
    ...
    const handleVideoClick = (e: React.MouseEvent) => {
        ...
        const isRapidTap = delta < 350 || (seekFeedback !== null && delta < 600);
        ...
        if (isRapidTap) {
            ...
            clickTimeoutRef.current = window.setTimeout(() => {
                const v = videoRef.current;
                if (v) {
                    const isPausedNow = v.paused;
                    if (isPausedNow) safePlay();
                    else safePause();
                    
                    setPlaybackFeedback(isPausedNow ? 'play' : 'pause');
    ...
    return (
        <div ref={observerRef} className="w-full h-full snap-start relative bg-black flex justify-center overflow-hidden">
            ...
            <div className="absolute right-3.5 bottom-28 z-[100] flex flex-col items-center gap-5 pointer-events-auto">
                ...
                <div onClick={(e) => { e.stopPropagation(); navigate(`/channel/@${short.profiles?.username?.toLowerCase()}`); }} className="w-11 h-11 rounded-full border-2 border-white/20 bg-slate-900 flex items-center justify-center animate-spin-slow shadow-xl overflow-hidden mt-1 p-0.5"><img src={short.profiles?.avatar_url} className="w-full h-full rounded-full object-cover" /></div>
            </div>

            <div className="absolute bottom-12 left-4 right-16 z-[100] pointer-events-none">
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-3">
                        <div onClick={(e) => { e.stopPropagation(); navigate(`/channel/@${short.profiles?.username?.toLowerCase()}`); }} className="pointer-events-auto flex items-center gap-2.5 bg-black/40 backdrop-blur-3xl px-2 py-1.5 pr-4 rounded-full border border-white/10 hover:bg-white/10 transition-all cursor-pointer group shadow-xl"><img src={short.profiles?.avatar_url} className="w-8 h-8 rounded-full object-cover border border-white/20" /><span className="text-[13px] font-black text-white italic tracking-tight flex items-center gap-1">@{channelName}<CheckCircle2 size={12} className="text-brand" fill="currentColor" /></span></div>
                        <SubscribeAction channelName={channelName} variant="shorts" />
                    </div>
                    <div className="space-y-2 pointer-events-auto cursor-pointer" onClick={(e) => { e.stopPropagation(); onDescriptionClick(); }}>
                        <p className="text-[14px] font-bold text-white leading-tight drop-shadow-lg pr-4 line-clamp-2 italic hover:underline decoration-white/20">
                            {short.caption}
                        </p>
                        <div className="flex flex-wrap gap-2 pointer-events-auto">
                            {short.tags?.map(t => (<span key={t} className="text-[10px] font-black text-brand bg-brand/10 border border-brand/20 px-3 py-1.5 rounded-xl uppercase tracking-widest">#{t}</span>))}
                        </div>
                    </div>
            ...
            {/* Fixed duplicate onMouseDown attribute and corrected to onMouseMove */}
