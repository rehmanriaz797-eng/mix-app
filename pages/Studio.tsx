...
import { getAnalytics, getUserChannel, getUploadedVideos, deleteVideo, updateVideo, getComments, saveCommunityPost, saveLiveStream } from '../services/storageService';
import { Channel, AnalyticsData, Video, Comment, CommunityPost } from '../types';
...
const Studio: React.FC = () => {
  ...
  const handleCreatePost = () => {
      ...
      alert("Neural Update Synchronized to Feed");
  };
  ...
  const handleGoLive = () => {
      ...
      const newStream = {
          id: `live_${Date.now()}`,
          user_id: user.id,
          title: "Neural Session: High Fidelity Transmission",
          thumbnail_url: "https://picsum.photos/seed/live/1280/720",
          viewer_count: 0,
          is_live: true,
          started_at: new Date().toISOString(),
          profiles: profile as any
      };
      saveLiveStream(newStream);
      alert("Live Stream Protocol Initiated");
      navigate('/channel');
  };
    ...
    <div className="flex min-h-screen bg-yt-base">
        ...
        <div className="flex-1 p-8 overflow-y-auto h-screen scrollbar-hide">
           <div className="flex items-center justify-between mb-10">
               <div>
                   ...
                   <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Real-time Intelligence Hub</p>
               </div>
               
               <div className="relative">
                   <button 
                        onClick={() => setShowCreateMenu(!showCreateMenu)}
                        className="bg-brand text-white px-8 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-600 transition-all flex items-center gap-3 shadow-xl shadow-brand/20"
                   >
                       <Plus size={18} /> Create Node
                   </button>
                   
                   {showCreateMenu && (
                       <div className="absolute top-full right-0 mt-3 w-64 bg-[#0d0d0d] border border-white/10 rounded-[2rem] shadow-[0_30px_100px_rgba(0,0,0,1)] overflow-hidden z-[100] animate-in fade-in zoom-in-95 duration-200">
                           <button onClick={() => { navigate('/upload'); setShowCreateMenu(false); }} className="w-full flex items-center gap-4 px-6 py-5 hover:bg-white/5 transition-colors group">
                               <VideoIcon size={20} className="text-slate-500 group-hover:text-brand" />
                               <span className="text-[11px] font-black uppercase tracking-widest">Upload Broadcast</span>
                           </button>
                           <button onClick={() => { handleGoLive(); setShowCreateMenu(false); }} className="w-full flex items-center gap-4 px-6 py-5 hover:bg-white/5 transition-colors group">
                               <Radio size={20} className="text-slate-500 group-hover:text-red-500" />
                               <span className="text-[11px] font-black uppercase tracking-widest">Initiate Live</span>
                           </button>
                           <button onClick={() => { setIsPostModalOpen(true); setShowCreateMenu(false); }} className="w-full flex items-center gap-4 px-6 py-5 hover:bg-white/5 transition-colors group">
                               <Type size={20} className="text-slate-500 group-hover:text-brand" />
                               <span className="text-[11px] font-black uppercase tracking-widest">Post Community</span>
                           </button>
                       </div>
                   )}
               </div>
        ...
        {isPostModalOpen && (
            <div className="fixed inset-0 z-[1000] bg-black/90 backdrop-blur-3xl flex items-center justify-center p-6 animate-in fade-in duration-300">
                <div className="bg-[#0b1026] w-full max-w-xl rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.9)] border border-white/5 flex flex-col max-h-[90vh] overflow-hidden">
                    <div className="flex justify-between items-center p-10 border-b border-white/5 bg-white/5">
                        <h2 className="text-3xl font-black uppercase tracking-tighter italic">Create Protocol Post</h2>
                        <button onClick={() => setIsPostModalOpen(false)} className="p-4 hover:bg-white/10 rounded-[1.5rem] transition-all"><X size={28} /></button>
                    </div>
                    <div className="p-10 space-y-8">
                        <textarea 
                            className="w-full bg-black border border-white/10 rounded-2xl p-6 focus:border-brand outline-none h-64 resize-none text-slate-300 font-medium leading-relaxed transition-all shadow-inner italic"
                            placeholder="Share resonance with your nodes..."
                            value={postContent}
                            onChange={(e) => setPostContent(e.target.value)}
                        />
                        <div className="flex justify-end gap-5">
                            <button onClick={() => setIsPostModalOpen(false)} className="px-10 py-5 text-[11px] font-black text-slate-600 uppercase tracking-[0.3em] hover:text-white transition-all">Abort</button>
                            <button onClick={handleCreatePost} className="px-12 py-5 bg-brand text-white font-black rounded-3xl text-[11px] uppercase tracking-[0.4em] shadow-2xl active:scale-95 transition-all">Broadcast Post</button>
                        </div>
                    </div>
                </div>
            </div>
