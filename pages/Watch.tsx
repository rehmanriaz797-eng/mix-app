...
import { 
    ...
    addToHistory, getVideoById, 
    getPlaylists, toggleVideoInPlaylist, createPlaylist 
...
import { 
    ThumbsUp, ThumbsDown, Share2, MessageSquare, Sparkles, 
    Loader2, Crown, Users, Gift, ListPlus, X, Clipboard
} from 'lucide-react';
...
const Watch: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const finalizeView = (v: Video) => {
        ...
        setHasDisliked(isVideoDisliked(v.id));
    };
    loadVideoData();
    window.scrollTo(0, 0);
    setAiInsight(null);
  ...
  const handleDislike = () => {
    if (!video) return;
    const nowDisliked = toggleDislikeVideo(video.id);
    setHasDisliked(nowDisliked);
    if (nowDisliked) {
  ...
  const handleChannelNav = () => {
    if (video) navigate(`/channel/${video.channelHandle}`);
  };
        ...
        <div className="mt-8 px-2 space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pt-4 border-t border-white/5">
                <div className="flex items-center gap-4">
                    <div className="relative cursor-pointer" onClick={handleChannelNav}>
                    ...
                    <div className="flex flex-col mr-8">
                        <span onClick={handleChannelNav} className="font-black text-lg text-white hover:text-brand cursor-pointer transition-colors">{video.channelName}</span>
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Creator Verified</span>
                    </div>
                    <div className="flex gap-2 items-center">
                        <SubscribeAction channelName={video.channelName} />
                        <button 
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center bg-white/5 border border-white/10 rounded-2xl h-12 overflow-hidden shadow-lg">
                        <button onClick={handleLike} className={`px-6 h-full flex items-center gap-2 hover:bg-white/5 transition-all active:scale-90 ${hasLiked ? 'text-brand' : 'text-slate-300'}`}>
                            <ThumbsUp size={18} fill="currentColor" stroke="none" /> 
                            <span className="font-black text-sm">{hasLiked ? (video.likes || 0) + 1 : video.likes || 0}</span>
                        </button>
                        <div className="w-px h-6 bg-white/10"></div>
                        <button onClick={handleDislike} className={`px-6 h-full flex items-center hover:bg-white/5 transition-all active:scale-90 ${hasDisliked ? 'text-white' : 'text-slate-300'} ${isDislikeAnimating ? 'animate-dislike-drop' : ''}`}>
